...
export default function SideBar({
  ...
  return (
    <div
      className={`
    flex flex-col h-full w-full text-[11.5px]
    transition-all duration-300 ease-in-out 
    gap-5 p-3 pt-2 bg-[#f6f7f9] overflow-hidden ${
      isSidebarOpen ? "min-w-[20vw]" : ""
    }
  `}
      style={{ height: isNavBarVisible ? "calc(100vh - 80px)" : "100%" }}
    >
      <div className="flex flex-col gap-4 shrink-0 h-[8%]">
        <SideBarHeader toggleSidebar={toggleSidebar} />
      </div>

      <div className="shrink-1 h-[92%] flex flex-col">
        {/* NEW LINE */}
        <div className="flex-1 flex flex-col justify-between overflow-hidden mt-[-15px]">
          <FilterSection
            title={
              allFundsSelected ? (
            ...
            ${screenSize === "medium" ? "max-h-[150px]" : ""}
            ${screenSize === "large" ? "max-h-[150px]" : ""}
          `}
